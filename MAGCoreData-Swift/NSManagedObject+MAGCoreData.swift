...
extension NSManagedObject {
    ...
    private struct AssociatedKey {
        static let KeyMapping = "NSManagedObjectMagCoreDataMappingKey"
        static let ValueTransformers = "NSManagedObjectValueTransformersKey"
        static let RelationClasses = "NSManagedObjectMagCoreDataRelationsKey"
        static let DatesFormats = "NSManagedObjectMagCoreDataDatesFormatKey"
        static let DefaultDateFormat = "NSManagedObjectMagCoreDataDefaultDateFormatKey"
        static let PrimaryKeyName = "NSManagedObjectMagCoreDataPrimaryKeyNameKey"
        static let UpdateDateKeyName = "NSManagedObjectMagCoreDataUpdateDateKey"
    }
    
    static var keyMapping: [String: String]? {
        get {
            return objc_getAssociatedObject(self, AssociatedKey.KeyMapping) as? [String: String]
        }
        set(keyMapping) {
            objc_setAssociatedObject(self, AssociatedKey.KeyMapping, keyMapping, UInt(OBJC_ASSOCIATION_COPY_NONATOMIC))
        }
    }
    ...
    static var updateDateKeyName: String? {
        get {
            return objc_getAssociatedObject(self, AssociatedKey.UpdateDateKeyName) as? String
        }
        set(updateDateKeyName) {
            objc_setAssociatedObject(self, AssociatedKey.UpdateDateKeyName, updateDateKeyName, UInt(OBJC_ASSOCIATION_COPY_NONATOMIC))
        }
    }
...
//        return allInContext(MAGCoreData.context, error: error)
    ...
    class func getOrCreateObjectForPrimaryKey(primaryKey: AnyObject, inContext context: NSManagedObjectContext, error: NSErrorPointer = nil) -> NSManagedObject? {
        if let primaryKeyName = primaryKeyName {
            if let managedObject = firstWithKey(primaryKeyName, value: primaryKey, inContext: context, error: error) {
    ...
    class func safeCreateOrUpdateWithDictionary(keyedValues: [String: AnyObject], inContext context: NSManagedObjectContext) -> NSManagedObject? {
        if let attributesNamesMap = keyMapping,
            ...
            primaryKeyAttributeDescription: AnyObject = entityDescription.attributesByName[primaryKeyName] {
                ...
                if let mappedPrimaryKey = attributesNamesMap[primaryKeyName], primaryKeyValue: AnyObject = keyedValues[mappedPrimaryKey] {
    ...
    private func createRelationshipToOneForRelationName(relationName: String, relationKey: String, attributes: [String: AnyObject], inContext context: NSManagedObjectContext) {
        if let relationshipDescription = entity.relationshipsByName[relationName] as? NSRelationshipDescription, relationClasses = self.dynamicType.relationClasses, objectClass: AnyClass = relationClasses[relationName] {
            if let managedObject = objectClass.safeCreateOrUpdateWithDictionary(attributes, inContext: context) {
